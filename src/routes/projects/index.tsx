...
function ProjectsIndex() {
  ...
  const [projectNameFromFile, setProjectNameFromFile] = useState<string | null>(
    null,
  );
  const [destinationDir, setDestinationDir] =
  ...
  const handleSelectDestination = async () => {
    try {
      ...
      setImportError(
        "Failed to select destination folder. Please try a different location.",
      );
    }
  };
  ...
  const handleStartImport = async () => {
    ...
    try {
      ...
      if (useProjectsFolder) {
        try {
          finalDestination = await destinationDir.getDirectoryHandle(
