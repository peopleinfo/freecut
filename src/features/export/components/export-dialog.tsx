...
export function ExportDialog({ open, onClose }: ExportDialogProps) {
  ...
  const [videoContainer, setVideoContainer] =
    useState<ClientVideoContainer>("mp4");
  const [audioContainer, setAudioContainer] =
    useState<ClientAudioContainer>("mp3");
  ...
  const [gpuInfo, setGpuInfo] = useState<FFmpegCapabilities | null>(null);

  // Check GPU capabilities when dialog opens
  useEffect(() => {
    if (open && !gpuInfo) {
      checkFFmpegCapabilities()
        .then(setGpuInfo)
  ...
  const exportRange = useMemo(() => {
    if (renderWholeProject || !hasInOutPoints) {
      return {
        start: 0,
        end: timelineDurationFrames,
        duration: timelineDurationFrames,
      };
    }
  ...
  const getVideoContainerOptions = () => [
    { value: "mp4", label: "MP4", description: "Most compatible, H.264/H.265" },
    {
      value: "mov",
      label: "QuickTime (MOV)",
      description: "Best for macOS/iOS",
    },
    { value: "webm", label: "WebM", description: "Web-optimized, VP8/VP9" },
    {
      value: "mkv",
      label: "Matroska (MKV)",
      description: "Flexible, all codecs",
    },
  ];
  ...
  useEffect(() => {
    ...
    if (!validCodecs.includes(settings.codec)) {
      setSettings((prev) => ({
        ...prev,
        codec: validCodecs[0] as ExportSettings["codec"],
      }));
    }
  }, [videoContainer]);
  ...
  const isVideoResult =
    clientRender.result?.mimeType?.startsWith("video/") ?? false;
            ...
            <div className="space-y-3 p-3 rounded-lg border border-border bg-muted/20">
              ...
              {hasInOutPoints && !renderWholeProject && (
                <p className="text-xs text-muted-foreground">
                  Exporting in/out range. Toggle above to export the full
                  timeline.
                </p>
              )}
            </div>
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        setVideoContainer(v as ClientVideoContainer)
                      }
                    >
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setSettings({
                          ...settings,
                          codec: value as ExportSettings["codec"],
                        })
                      }
                    >
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setAudioContainer(v as ClientAudioContainer)
                    }
                  >
            ...
            {exportMode === "video" && gpuInfo && (
              <div className="flex items-center gap-2 p-2.5 rounded-lg border border-border bg-muted/20">
                {gpuInfo.available && gpuInfo.hwAccel.available ? (
                  ...
                  <>
                    <Cpu className="h-4 w-4 text-yellow-500 flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <span className="text-sm font-medium text-yellow-400">
                        Software Encoding
                      </span>
                      <span className="text-xs text-muted-foreground ml-2">
                        FFmpeg {gpuInfo.version}
                      </span>
                    </div>
                  </>
                ) : (
        ...
        {view === "progress" && (
          <div className="space-y-4 py-4 overflow-hidden">
            <div className="space-y-4 min-w-0">
              ...
              <p className="text-xs text-muted-foreground">
                Keep this tab open while rendering. Longer videos may take
                several minutes.
              </p>
            </div>
          ...
          <div className="space-y-4 py-4">
            ...
            <Alert className="border-green-900 bg-green-950">
              ...
              <AlertDescription className="text-green-400">
                {exportMode === "audio" ? "Audio" : "Video"} exported
                successfully!
              </AlertDescription>
            </Alert>
        ...
        {view === "cancelled" && (
          <div className="space-y-4 py-4">
            <Alert>
              <X className="h-4 w-4" />
              <AlertDescription>
                The export process was cancelled.
              </AlertDescription>
            </Alert>
